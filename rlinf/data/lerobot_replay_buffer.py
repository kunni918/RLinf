...
def _decode_image_path(value: Any, frame: dict[str, Any]) -> torch.Tensor:
    ...
    candidate_paths = [image_path] if image_path.is_absolute() else []
    dataset_root = frame.get("_lerobot_dataset_root")
    if dataset_root is not None and not image_path.is_absolute():
        candidate_paths.append(Path(str(dataset_root)) / image_path)
    for candidate_path in candidate_paths:
        if candidate_path.is_file():
            return _decode_image_bytes(candidate_path.read_bytes())
...
def _image_tensor(value: Any, frame: dict[str, Any]) -> torch.Tensor:
    if isinstance(value, dict):
        ...
        if value.get("bytes") is not None:
            return _decode_image_bytes(value["bytes"])
        if value.get("path") is not None:
            return _decode_image_path(value["path"], frame)
    ...
    tensor = _try_as_tensor(value)
    if tensor is None:
        raise ValueError("Image value cannot be converted to a tensor.")
...
def _bool_from_frame(frame: dict[str, Any], keys: Sequence[str], default: bool) -> bool:
    value = _find_value(frame, keys)
    if value is None:
        return default
    return bool(_scalar(value, default))
...
def _combine_optional_bool(
    ...
    *,
    ...
    values = [_optional_bool(source_frame, keys, field_name=field_name)]
    if target_frame is not None:
        values.append(_optional_bool(target_frame, keys, field_name=field_name))
...
def _terminal_flags_from_transition(
    ...
    *,
    ...
    done_value = _combine_optional_bool(
        source_frame, target_frame, _DONE_KEYS, field_name="done"
    )
    termination_value = _combine_optional_bool(
        source_frame, target_frame, _TERMINATION_KEYS, field_name="terminated"
    )
    truncation_value = _combine_optional_bool(
        source_frame, target_frame, _TRUNCATION_KEYS, field_name="truncated"
    )
    ...
    split_done = _known_split_done(termination_value, truncation_value)
    has_terminal_metadata = _has_terminal_metadata(
        source_frame
    ) or _has_terminal_metadata(target_frame)
    if (
        default_done
    ...
    if target_frame is not None and default_done and done and split_done is False:
...
def _reward_from_transition(
    ...
    *,
    ...
    if reward_value is not None:
        return float(_scalar(reward_value, 0.0))
    success = any(
        _bool_from_frame(source_frame, (key,), False) for key in _SUCCESS_KEYS
    )
    if target_frame is not None:
        success = success or any(
            _bool_from_frame(target_frame, (key,), False) for key in _SUCCESS_KEYS
        )
...
def _find_lerobot_dataset_roots(root: Path) -> list[Path]:
    ...
    data_dirs = {
        ...
        if "data" in parquet_file.relative_to(root).parts
    }
    dataset_roots = set()
    for data_dir in data_dirs:
        parts = data_dir.relative_to(root).parts
        data_index = parts.index("data")
        dataset_roots.add(root.joinpath(*parts[:data_index]))
...
def _validate_unique_frame_indices(
    frames: Sequence[dict[str, Any]], *, dataset_root: Path
) -> None:
    seen: set[tuple[int, int]] = set()
    for frame in frames:
        episode_index, frame_index = _frame_order(frame)
        key = (episode_index, frame_index)
        if key in seen:
            raise ValueError(
                f"Duplicate frame_index={frame_index} in episode_index={episode_index} "
...
def load_lerobot_parquet_frames(
    ...
    for dataset_root in dataset_roots:
        ...
        _validate_unique_frame_indices(root_frames, dataset_root=dataset_root)
        ...
        for frame in sorted(root_frames, key=_frame_order):
            ...
            converted_frame["_lerobot_episode_index"] = local_episode_index
