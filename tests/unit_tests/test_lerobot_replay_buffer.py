...
def test_load_lerobot_parquet_frames_rejects_duplicate_frame_indices(tmp_path):
    pandas = pytest.importorskip("pandas")
    data_dir = tmp_path / "data" / "chunk-000"
    data_dir.mkdir(parents=True)
    pandas.DataFrame(
...
def test_realworld_chunk_step_helper_import_does_not_import_realworld_package(
    ...
    assert callable(run_realworld_chunk_step)
...
def test_validate_cfg_rejects_only_eval_sac_training_model_before_cluster_init():
    ...
    with patch("rlinf.config.Cluster", side_effect=AssertionError("Cluster called")):
        with pytest.raises(ValueError, match="use_dsrl"):
            validate_cfg(cfg, require_embodied_sac_training_model=True)
...
def test_sac_demo_buffer_rejects_conflicting_legacy_load_distributed():
    from rlinf.config import validate_demo_buffer_load_mode

    with pytest.raises(ValueError, match="load_distributed"):
        validate_demo_buffer_load_mode(
            OmegaConf.create({"load_mode": "replicate", "load_distributed": True})
        )
